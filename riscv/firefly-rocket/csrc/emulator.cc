...
\n\
FIREFLY OPTIONS\n\
  -i, --voltage-input      Voltage trace used in simulation. If not specified, \n\
                           Firefly assume that a file named \"voltage-input.txt\" \n\
                           exists and will try to use it \n\
  -t, --voltage-threshold  Whenever voltage is below the threshold, a interrupt \n\
                           is triggered and Rocket will save its context in memory \n\
  -d, --output-dir         Save all simulation generated files in the specified \n\
                           directory. If not specified, the current directory will \n\
                           be used as output directory \n\
", stdout);
#if VM_TRACE == 0
  fputs("\
\n\
...
// Firefly args
  float vt = 0.07f;
  std::string ff_report = "firefly_report.log";
  std::string voltage_input = "voltage_input.txt";
  std::string out_dir = "./";
  ...
  while (1) {
    static struct option long_options[] = {
      ...
      {"voltage-input",     required_argument, 0, 'i' },
      {"voltage-threshold", required_argument, 0, 't' },
      {"firefly-log",       required_argument, 0, 'l' },
    ...
    int option_index = 0;
#if VM_TRACE
    int c = getopt_long(argc, argv, "-chm:s:r:v:Vx:i:t:l:d:", long_options, &option_index);
#else
    int c = getopt_long(argc, argv, "-chm:s:r:Vi:t:l:d:", long_options, &option_index);
#endif
    if (c == -1) break;
 retry:
    switch (c) {
      ...
      case 'i': voltage_input = optarg;     break;
      case 't': vt = atof(optarg);          break;
      case 'l': ff_report = optarg;         break;
...
done_processing:
  ...
  // Firefly
  firefly* ff = new firefly(voltage_input, tile, &trace_count, vt);
  tile->io_sim_int = 0;
  ...
  while (trace_count < max_cycles) {
    ...
    tile->clock = 0;
    tile->reset = reset_count < async_reset_cycles*2 ? reset_count % 2 :
      reset_count < async_reset_cycles*2 + sync_reset_cycles;
    ...
    tile->clock = reset_count >= async_reset_cycles*2;
    tile->eval();
#if VM_TRACE
    if (dump)
  ...
  ff->save_report(out_dir + "/" + ff_report);
  std::string ff_mem_trace = out_dir + "/" + "mem_trace.nvmain";
  ff->save_mem_trace(ff_mem_trace);
